  ...
  for i,ptbin in enumerate(ptedges):
    ...
    for c in collections_list:
      ...
      for identifier in c['identifiers']:
        incl_sigmas.append(pickle.load(open(options.submitDir+'/'+'incl_sigmas_'+identifier+'.p','rb'))[i-1])
        incl_sigma_errs.append(pickle.load(open(options.submitDir+'/'+'incl_sigma_errs_'+identifier+'.p','rb'))[i-1])
    ...
    for c in collections_list:
      ...
      incl_sigma_errs = []
      for identifier in c['identifiers']:
        incl_sigmas.append(pickle.load(open(options.submitDir+'/'+'incl_sigmaRs_'+identifier+'.p','rb'))[i-1])
        incl_sigma_errs.append(pickle.load(open(options.submitDir+'/'+'incl_sigmaR_errs_'+identifier+'.p','rb'))[i-1])
