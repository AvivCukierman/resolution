...
def fitres(params=[]):
  ...
  has_widths = False
  filename = options.submitDir+'/'+'widths_'+options.identifier+'.npy'
  if os.path.exists(filename):
    if not options.root: print '== Loading file <'+filename+'> as reco jet widths =='
    widths = load(filename)
    if not len(widths)==len(truepts):
      raise RuntimeError('There should be the same number of widths as truth jets')
    has_widths = True
  else:
  ...
  for npvbin in xrange(1,len(npvedges)):
    ...
    for ptbin in xrange(1,len(ptedges)): 
      ...
      ax = plt.gca()
      plt.text(0.5,0.9,r'$\mu=$'+str(mu)+r'$\pm$'+str(mu_err),transform=ax.transAxes)
      plt.text(0.5,0.8,r'$\sigma=$'+str(sigma)+r'$\pm$'+str(sigma_err),transform=ax.transAxes)
    ...
    plt.xlim(0,options.maxpt+10)
    ax = plt.gca()
    plt.text(0.05,0.9,r'Fit: '+str(Ropt[0])+'+'+str(Ropt[1])+'/log(x+'+str(asym)+')+'+str(Ropt[2])+'/log(x+'+str(asym)+'$)^2$',transform=ax.transAxes,bbox={'facecolor':'white', 'alpha':1.0, 'pad':10})
    #result = a + b/log(ax+asym) + c/log(ax+asym)**2
    ...
    for ptbin in xrange(1,len(ptedges)): 
      ...
      plt.ylim(0,1.5)
      ax = plt.gca()
      plt.text(0.5,0.9,r'$\mu=$'+str(muR)+r'$\pm$'+str(muR_err),transform=ax.transAxes)
      plt.text(0.5,0.8,r'$\sigma=$'+str(sigmaR)+r'$\pm$'+str(sigmaR_err),transform=ax.transAxes)
